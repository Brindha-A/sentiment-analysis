...
def clickMe():
    user_input = name.get()
    ans = prediction.predict(user_input)
    print(ans)
...
label = ttk.Label(window, text="Sentiment Analysis", font=titleStyle)
label.grid(column=0, row=0)
label.place(x=400, y=50, anchor="center")

# label = ttk.Label(window, text="Enter the Sentence", font=fontStyle)
# label.grid(column=0, row=0)
# label.place(x=400, y=120, anchor="center")
